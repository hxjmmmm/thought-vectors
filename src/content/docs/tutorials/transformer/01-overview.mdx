...
2. **长距离依赖弱**：句子长了，前面的信息传到后面就"衰减"了。LSTM 试图解决这个问题，但效果有限。

2017 年，Google 的论文 [Attention Is All You Need](https://arxiv.org/abs/1706.03762) 提出了 Transformer，**完全抛弃了循环结构，只用注意力机制**。这个设计让模型可以并行计算，同时任意两个位置的词都能直接"看到"彼此。

> 💡 这篇论文是目前整个大模型时代的起点。ChatGPT、Claude、Gemini 的底层架构全部源自这里。
